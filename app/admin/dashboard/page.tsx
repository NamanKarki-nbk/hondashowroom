    ...
    <div className="min-h-screen bg-[#f3ebdd] dark:bg-[#0B0B0C] text-gray-100 p-8 selection:bg-[#c1291A] selection:text-[#f3ebdd]">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex justify-between items-end mb-10">
          <div>
            <h1 className="text-3xl font-extrabold text-[#f3ebdd] tracking-tight">Executive Dashboard</h1>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl px-4 py-2 flex items-center gap-3 backdrop-blur-md">
            <CalendarIcon className="w-5 h-5 text-[#c1291A]" />
            <span className="text-sm font-medium text-[#f3ebdd]">August 2026</span>
          </div>
        </header>
          ...
          {KPI_DATA.map((kpi, idx) => (
            <div key={idx} className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 backdrop-blur-md relative overflow-hidden group hover:border-slate-700 transition-colors">
               <div className="flex justify-between items-start mb-4">
                  <div className="p-3 bg-[#f3ebdd]/5 rounded-xl text-gray-400 group-hover:text-[#c1291A] group-hover:bg-[#c1291A]/10 transition-colors">
               ...
               <h3 className="text-3xl font-black text-[#f3ebdd] mb-1">{kpi.value}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-slate-900/40 border border-slate-800 rounded-3xl p-8 backdrop-blur-md">
            <h2 className="text-xl font-bold text-[#f3ebdd] mb-6">Inventory: Color Variant Matrix</h2>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-800 text-xs uppercase tracking-wider text-gray-500">
                    ...
                    <th className="pb-4 font-semibold text-center"><div className="w-4 h-4 rounded-full bg-[#c1291A] mx-auto mb-1"></div>Red</th>
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {COLOR_MATRIX.map((row, idx) => (
                    <tr key={idx} className="hover:bg-[#f3ebdd]/5 transition-colors">
                      <td className="py-4 text-[#f3ebdd] font-medium">{row.model}</td>
                      ...
                      <td className="py-4 text-right text-[#f3ebdd] font-bold">{row.red + row.black + row.matte}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 backdrop-blur-md">
            <h2 className="text-xl font-bold text-[#f3ebdd] mb-6">Upcoming Deliveries</h2>
            <div className="space-y-4">
               {[
                 ...
                 <div key={idx} className="flex justify-between items-center bg-black/40 p-4 rounded-xl border border-[#f3ebdd]/5">
                    <div>
                       <p className="text-sm font-bold text-[#f3ebdd]">{delivery.customer}</p>
                       <p className="text-xs text-[#c1291A] font-medium">{delivery.model}</p>
                    </div>
                    <div className="text-xs text-gray-500 font-mono bg-[#f3ebdd]/5 px-2 py-1 rounded">
                       {delivery.date}
                    </div>
                 </div>
               ))}
            </div>
            <button className="w-full mt-6 bg-[#f3ebdd]/5 hover:bg-[#f3ebdd]/10 text-[#f3ebdd] py-3 rounded-xl text-sm font-semibold transition-colors">
               View Full Calendar
            </button>
          </div>
