...
export default async function AdminPricesPage() {
  ...
  const prices = await prisma.vehicleVariant.findMany({
    orderBy: { createdAt: "desc" },
    include: { vehicleMaster: true } // Need this for the client UI
  });

  const costPrices: Record<string, number> = {};
  inventoryAggregation.forEach((agg) => {
    if (agg._avg?.purchasePrice) {
      costPrices[agg.variantId] = agg._avg.purchasePrice;
    }
  });
