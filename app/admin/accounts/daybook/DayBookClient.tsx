...
type Receipt = {
  ...
  transaction: {
    ...
    vehicle: { variant: { vehicleMaster: { name: string } } };
  };
};
...
export default function DayBookClient({
  ...
  const navigateDate = (direction: "prev" | "next" | "today") => {
    ...
    if (direction === "today") newDate = defaultDate;
    else if (direction === "prev") newDate = format(subDays(new Date(selectedDate + "T12:00:00"), 1), "yyyy-MM-dd");
    else newDate = format(addDays(new Date(selectedDate + "T12:00:00"), 1), "yyyy-MM-dd");
    startTransition(() => router.push(`/admin/accounts/daybook?date=${newDate}`));
  };
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-slate-900/50 border-b border-gray-100 dark:border-slate-800">
                  <th className="py-3 px-5 text-xs font-bold uppercase tracking-widest text-gray-500">Receipt No.</th>
                  <th className="py-3 px-5 text-xs font-bold uppercase tracking-widest text-gray-500">Customer</th>
                  <th className="py-3 px-5 text-xs font-bold uppercase tracking-widest text-gray-500">Vehicle</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800/50">
                {receipts.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50/80 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-3 px-5 font-mono text-xs text-gray-700 dark:text-gray-300">{r.receiptNo}</td>
                    <td className="py-3 px-5 font-bold text-gray-900 dark:text-white text-sm">{r.transaction.customer.fullName}</td>
                    <td className="py-3 px-5 text-sm text-gray-600 dark:text-gray-400">{r.transaction.vehicle.variant.vehicleMaster.name}</td>
                    <td className="py-3 px-5">
                      <span className="text-xs font-bold bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-full">{r.paymentMethod}</span>
                    </td>
                    <td className="py-3 px-5 text-right font-black text-emerald-600 dark:text-emerald-400">{formatNPR(r.amount)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t-2 border-emerald-200 dark:border-emerald-900/30">
                <tr>
                  <td colSpan={4} className="py-3 px-5 text-right font-black text-gray-900 dark:text-white uppercase tracking-wider text-sm">Total Income:</td>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[600px]">
              ...
              <tfoot className="border-t-2 border-red-200 dark:border-red-900/30">
                <tr>
                  <td colSpan={2} className="py-3 px-5 text-right font-black text-gray-900 dark:text-white uppercase tracking-wider text-sm">Total Expenses:</td>
