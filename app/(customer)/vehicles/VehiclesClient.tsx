...
export default function VehiclesClient({ products }: { products: any[] }) {
  ...
  return (
    <div className="min-h-screen bg-background dark:bg-[#0B0B0C] text-gray-100 pt-24 pb-20 px-6 selection:bg-primary selection:text-primary-foreground relative">
      {/* Modals */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-background dark:bg-slate-900 w-full max-w-md rounded-3xl p-6 shadow-2xl border border-gray-200 dark:border-slate-800 relative">
            <button 
              ...
              className="absolute top-4 right-4 text-gray-500 hover:text-[#B83227]"
            >
            ...
            <form className="space-y-4" onSubmit={e => { e.preventDefault(); setModalType(null); setSelectedVehicle(null); alert("Submitted successfully!"); }}>
              <div>
                ...
                <input type="text" required placeholder="Your Name" className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-[#B83227]" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider ml-1">Phone Number</label>
                <input type="tel" required placeholder="Mobile Number" className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-[#B83227]" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider ml-1">Preferred Dealer</label>
                <select className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-[#B83227]">
              ...
              <button type="submit" className="w-full bg-[#B83227] hover:bg-primary-hover text-white py-3 rounded-xl font-bold uppercase tracking-wider mt-2 transition-colors">
                Submit Request
              </button>
            </form>
          </div>
          ...
          <div className="w-full md:w-auto flex flex-col sm:flex-row gap-3">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full sm:w-64 bg-white dark:bg-slate-900/60 border border-gray-300 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-primary-foreground focus:ring-1 focus:ring-[#B83227] focus:border-[#B83227] outline-none backdrop-blur-sm"
              />
            </div>
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-sm font-bold uppercase tracking-wider transition-all ${
                selectedCategory === cat 
                  ? "bg-[#B83227] text-white shadow-lg shadow-[#B83227]/30 border border-[#B83227]" 
