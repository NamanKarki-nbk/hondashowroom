...
export default function GaragePage() {
  ...
  return (
    <div className="min-h-screen bg-background dark:bg-[#0B0B0C] text-gray-100 pt-24 pb-20 px-6 selection:bg-primary selection:text-primary-foreground overflow-x-hidden">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-8 backdrop-blur-md flex flex-col md:flex-row items-center gap-6 relative overflow-hidden shadow-2xl">
          ...
          <div className="w-24 h-24 bg-black border-2 border-primary rounded-full flex items-center justify-center relative z-10 shrink-0">
          ...
          <button className="relative z-10 bg-background/5 border border-slate-700 hover:bg-background/10 px-5 py-3 rounded-xl font-medium transition-colors">
            Edit Profile
          </button>
        </div>
               ...
               <div className="space-y-10">
                  {activeVehicle.history.map((record, idx) => (
                     <div key={idx} className="relative pl-12">
                        {/* Timeline node */}
                        <div className="absolute left-0 top-1.5 w-3 h-3 bg-primary rounded-full shadow-[0_0_10px_#c1291A] -ml-[5px]"></div>
                        
                        <div className="bg-black/50 border border-background/5 rounded-2xl p-5 hover:border-background/10 transition-colors group">
                           <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                              <div>
                                 <h4 className="text-lg font-bold text-primary-foreground mb-1">{record.type}</h4>
                              ...
                              <div className="text-left sm:text-right shrink-0">
                                 <div className="text-lg font-bold text-primary-foreground mb-2">Rs. {record.bill}</div>
                                 <button className="bg-background/5 hover:bg-background/10 text-xs font-semibold px-3 py-2 rounded-lg border border-background/10 transition-colors flex items-center justify-center sm:justify-end gap-1 w-full sm:w-auto">
