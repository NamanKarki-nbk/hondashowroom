      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#c1291A] via-[#c0151f] to-[#8b0000] py-14 px-6 mb-12">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <nav className="flex items-center justify-center gap-1.5 text-[#f3ebdd]/70 text-sm mb-6">
            ...
            <span className="text-[#f3ebdd] font-medium">Test Ride</span>
          </nav>
          <div className="flex items-center justify-center gap-3 mb-4">
            <Bike className="w-10 h-10 text-[#f3ebdd]/90" />
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {FEATURES.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="bg-[#f3ebdd] dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-[#f3ebdd]/10 p-6 text-center shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
            >
              <div className="w-12 h-12 rounded-full bg-[#c1291A]/10 flex items-center justify-center mx-auto mb-4">
                <Icon className="w-6 h-6 text-[#c1291A]" />
              </div>
              <h3 className="font-bold text-gray-900 dark:text-[#f3ebdd] text-sm mb-1.5">{title}</h3>
        ...
        {submitted ? (
          ...
          <div className="bg-[#f3ebdd] dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-[#f3ebdd]/10 shadow-xl overflow-hidden">
            <div className="bg-gradient-to-br from-emerald-500 to-green-600 p-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-[#f3ebdd] mb-1">Test Ride Scheduled!</h2>
              <p className="text-[#f3ebdd]/80 text-sm">We look forward to seeing you at the showroom.</p>
            </div>
            <div className="p-8 text-center">
              <div className="inline-flex flex-col items-center bg-[#f3ebdd] dark:bg-[#1a1a1a] rounded-xl px-8 py-5 mb-6 border border-gray-200 dark:border-[#f3ebdd]/10">
                ...
                <span className="text-2xl font-black text-[#c1291A] tracking-wider">{rideId}</span>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className={labelClass} htmlFor="model">
                  <span className="flex items-center gap-1.5">
                    <Bike className="w-3.5 h-3.5 text-[#c1291A]" /> Preferred Model
                  </span>
                </label>
                <select
                  id="model"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={labelClass} htmlFor="preferredDate">
                    <span className="flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5 text-[#c1291A]" /> Preferred Date
                    </span>
                  </label>
                  <input
                    id="preferredDate"
                ...
                <div>
                  <label className={labelClass} htmlFor="preferredTime">
                    <span className="flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-[#c1291A]" /> Preferred Time
                    </span>
                  </label>
                  <select
                    id="preferredTime"
              ...
              <div>
                <label className={labelClass} htmlFor="location">
                  <span className="flex items-center gap-1.5">
                    <MapPin className="w-3.5 h-3.5 text-[#c1291A]" /> Your Location / Address
                  </span>
                </label>
                <input
                  id="location"
              ...
              <div>
                <label className={labelClass} htmlFor="notes">
                  <span className="flex items-center gap-1.5">
                    <FileText className="w-3.5 h-3.5 text-[#c1291A]" /> Additional Notes
                  </span>
                </label>
                <textarea
                  id="notes"
